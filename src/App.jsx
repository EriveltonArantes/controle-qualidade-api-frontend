...
const apiBaseUrl = "https://controle-qualidade-api.onrender.com";
    ...
    <div className="login-screen">
      <div className="login-institucional">
        ...
        <ul className="login-features">
          ...
          <li>✓ Controle completo de Lote Producao</li>
          <li>✓ Controle completo de Inspecao Qualidade</li>
          <li>✓ Controle completo de Nao Conformidade</li>
        </ul>
      </div>
...
function PainelProduto({ token, apiBaseUrl, onPix, role, onLogout }) {
  ...
  const itensFiltrados = itens.filter(item => !busca || ["nome", "descricao", "categoria", "unidade", "peso", "especificacoes", "ativo", "id"].some(k => String(item[k] ?? "").toLowerCase().includes(busca.toLowerCase())));
  ...
  const editar = (item) => {
    const f = {};
    ["nome", "descricao", "categoria", "unidade", "peso", "especificacoes", "ativo"].forEach(k => { f[k] = item[k] ?? ""; });
...
function PainelLoteProducao({ token, apiBaseUrl, onPix, role, onLogout }) {
  const [itens, setItens] = React.useState([]);
  const [carregando, setCarregando] = React.useState(true);
  const [form, setForm] = React.useState({});
  const [editId, setEditId] = React.useState(null);
  ...
  // não muda o contrato da API (que os outros 9 projetos já usam tal como
  // está), só corta o que já foi carregado em páginas de 10.
  const itensFiltrados = itens.filter(item => !busca || ["produtoId", "numero", "dataProducao", "quantidade", "status", "observacoes", "id"].some(k => String(item[k] ?? "").toLowerCase().includes(busca.toLowerCase())));
  const totalPaginas = Math.max(1, Math.ceil(itensFiltrados.length / 10));
  const paginaAtual = Math.min(pagina, totalPaginas);
  const itensPagina = itensFiltrados.slice((paginaAtual - 1) * 10, paginaAtual * 10);

  const carregar = () => fetch(apiBaseUrl + "/api/loteproducaos", { headers: authHeaders(token) })
    .then(r => { if (r.status === 401) { onLogout && onLogout(); return []; } return r.json(); })
    .then(data => { if (Array.isArray(data)) setItens(data); })
    .catch(() => setItens([]))
    .finally(() => setCarregando(false));
  ...
  const salvar = (e) => {
    ...
    setErroSalvar("");
    setSalvando(true);
    const url = editId ? apiBaseUrl + "/api/loteproducaos/" + editId : apiBaseUrl + "/api/loteproducaos";
    fetch(url, {
      method: editId ? "PUT" : "POST",
      headers: { "Content-Type": "application/json", ...authHeaders(token) },
      body: JSON.stringify(montarCorpo()),
  ...
  const editar = (item) => {
    const f = {};
    ["produtoId", "numero", "dataProducao", "quantidade", "status", "observacoes"].forEach(k => { f[k] = item[k] ?? ""; });
    ["produto"].forEach(k => { f[k] = item[k + "Id"] ?? ""; });
    setErroSalvar(""); setForm(f); setEditId(item.id); setModalAberto(true);
  };

  const apagar = (id) => fetch(apiBaseUrl + "/api/loteproducaos/" + id, { method: "DELETE", headers: authHeaders(token) })
    .then(async r => {
      if (r.status === 401) { onLogout && onLogout(); return; }
      if (!r.ok) {
        const corpo = await r.json().catch(() => ({}));
  ...
  return (
    <div>
            ...
            <div className="kanban-coluna" key={coluna}>
              ...
              {itensFiltrados.filter(i => String(i.status ?? "Sem status") === coluna).map(item => (
                <div className="kanban-card" key={item.id}>
                  <div className="kanban-card-titulo">{String(item["numero"] ?? "LoteProducao")}</div>
                  <select className="kanban-select" value={item.status ?? ""} onChange={e => {
                    fetch(apiBaseUrl + "/api/loteproducaos/" + item.id, {
      ...
      {!modoKanban && <><div className="items-grid">
        {itensPagina.map(item => (
          <div className="item-card" key={item.id}>
            ...
            <div className="item-title">{String(item["numero"] ?? "LoteProducao")}</div>
      ...
      {modalAberto && (
        <div className="modal-overlay" onClick={() => setModalAberto(false)}>
          <div className="modal" onClick={e => e.stopPropagation()}>
            <h3>{editId ? "Editar" : "Novo"} LoteProducao</h3>
            {erroSalvar && <div className="campo-erro modal-erro">{erroSalvar}</div>}
            <form onSubmit={salvar}>
...
function PainelInspecaoQualidade({ token, apiBaseUrl, onPix, role, onLogout }) {
  const [itens, setItens] = React.useState([]);
  const [carregando, setCarregando] = React.useState(true);
  const [form, setForm] = React.useState({});
  const [editId, setEditId] = React.useState(null);
  ...
  const [busca, setBusca] = React.useState("");
  const [pagina, setPagina] = React.useState(1);
  const [loteProducaoList, setLoteProducaoList] = React.useState([]);

  // Paginação real (achado real auditando como cliente exigente: lista sem
  // limite nenhum, "queria ver pelo menos os 10 primeiros"). Front-end só —
  // não muda o contrato da API (que os outros 9 projetos já usam tal como
  // está), só corta o que já foi carregado em páginas de 10.
  const itensFiltrados = itens.filter(item => !busca || ["loteId", "inspector", "dataInspecao", "totalAmostras", "aprovadas", "reprovadas", "resultado", "observacoes", "id"].some(k => String(item[k] ?? "").toLowerCase().includes(busca.toLowerCase())));
  const totalPaginas = Math.max(1, Math.ceil(itensFiltrados.length / 10));
  const paginaAtual = Math.min(pagina, totalPaginas);
  const itensPagina = itensFiltrados.slice((paginaAtual - 1) * 10, paginaAtual * 10);

  const carregar = () => fetch(apiBaseUrl + "/api/inspecaoqualidades", { headers: authHeaders(token) })
    .then(r => { if (r.status === 401) { onLogout && onLogout(); return []; } return r.json(); })
    .then(data => { if (Array.isArray(data)) setItens(data); })
    .catch(() => setItens([]))
    .finally(() => setCarregando(false));
  ...
  React.useEffect(() => {
    carregar();
    fetch(apiBaseUrl + "/api/loteproducaos", { headers: authHeaders(token) }).then(r => r.json()).then(setLoteProducaoList).catch(() => {});
  }, [token]);
  ...
  const salvar = (e) => {
    ...
    setErroSalvar("");
    setSalvando(true);
    const url = editId ? apiBaseUrl + "/api/inspecaoqualidades/" + editId : apiBaseUrl + "/api/inspecaoqualidades";
    fetch(url, {
      method: editId ? "PUT" : "POST",
      headers: { "Content-Type": "application/json", ...authHeaders(token) },
      body: JSON.stringify(montarCorpo()),
  ...
  const editar = (item) => {
    const f = {};
    ["loteId", "inspector", "dataInspecao", "totalAmostras", "aprovadas", "reprovadas", "resultado", "observacoes"].forEach(k => { f[k] = item[k] ?? ""; });
    ["lote"].forEach(k => { f[k] = item[k + "Id"] ?? ""; });
    setErroSalvar(""); setForm(f); setEditId(item.id); setModalAberto(true);
  };

  const apagar = (id) => fetch(apiBaseUrl + "/api/inspecaoqualidades/" + id, { method: "DELETE", headers: authHeaders(token) })
    .then(async r => {
      if (r.status === 401) { onLogout && onLogout(); return; }
      if (!r.ok) {
        const corpo = await r.json().catch(() => ({}));
  ...
  return (
    <div>
      ...
      {true && <><div className="items-grid">
        {itensPagina.map(item => (
          <div className="item-card" key={item.id}>
            ...
            <div className="item-title">{String(item["inspector"] ?? "InspecaoQualidade")}</div>
            
            <div className="item-meta-grid">
            <div className="item-field"><b>loteId</b><span>{item["loteId"] ?? "—"}</span></div>
            <div className="item-field"><b>dataInspecao</b><span>{item["dataInspecao"] ? new Date(item["dataInspecao"]).toLocaleString("pt-BR", {dateStyle:"short",timeStyle:"short"}) : "—"}</span></div>
            <div className="item-field"><b>totalAmostras</b><span>{item["totalAmostras"] ?? "—"}</span></div>
            <div className="item-field"><b>aprovadas</b><span>{item["aprovadas"] ?? "—"}</span></div>
            <div className="item-field"><b>reprovadas</b><span>{item["reprovadas"] ?? "—"}</span></div>
            <div className="item-field"><b>resultado</b><span>{item["resultado"] ?? "—"}</span></div>
            <div className="item-field"><b>observacoes</b><span>{item["observacoes"] ?? "—"}</span></div>
            <div className="item-field"><b>lote</b><span>{item.loteId ? ((loteProducaoList.find(o => o.id === item.loteId) || {}).numero ?? ("#" + item.loteId)) : "—"}</span></div>
            </div>
            <div className="item-actions">
              <button className="btn btn-ghost btn-sm" onClick={() => editar(item)}>Editar</button>
              {role === "ADMIN" && <button className="btn btn-danger btn-sm" onClick={() => { if(window.confirm("Confirmar exclusão?")) apagar(item.id); }}>Apagar</button>}
      ...
      {modalAberto && (
        <div className="modal-overlay" onClick={() => setModalAberto(false)}>
          <div className="modal" onClick={e => e.stopPropagation()}>
            <h3>{editId ? "Editar" : "Novo"} InspecaoQualidade</h3>
            {erroSalvar && <div className="campo-erro modal-erro">{erroSalvar}</div>}
            <form onSubmit={salvar}>
              <label className="field-label">loteId</label>
      <input type="number" value={form["loteId"] ?? ""} onChange={e => setForm({...form, loteId: e.target.value})} />
      ...
      <label className="field-label">dataInspecao</label>
      <input type="datetime-local" value={form["dataInspecao"] ?? ""} onChange={e => setForm({...form, dataInspecao: e.target.value})} />
      <label className="field-label">totalAmostras</label>
      <input type="number" value={form["totalAmostras"] ?? ""} onChange={e => setForm({...form, totalAmostras: e.target.value})} />
      <label className="field-label">aprovadas</label>
      <input type="number" value={form["aprovadas"] ?? ""} onChange={e => setForm({...form, aprovadas: e.target.value})} />
      <label className="field-label">reprovadas</label>
      <input type="number" value={form["reprovadas"] ?? ""} onChange={e => setForm({...form, reprovadas: e.target.value})} />
      ...
      <select value={form["lote"] ?? ""} onChange={e => setForm({...form, lote: e.target.value})}>
        <option value="">Selecione...</option>
        {(loteProducaoList || []).map(o => (<option key={o.id} value={o.id}>{o.numero ?? ("#" + o.id)}</option>))}
      </select>
              <div className="modal-actions">
                <button type="submit" className="btn btn-primary" disabled={salvando}>{salvando ? "Salvando..." : "Salvar"}</button>
                <button type="button" className="btn btn-ghost" onClick={() => setModalAberto(false)} disabled={salvando}>Cancelar</button>
...
function PainelNaoConformidade({ token, apiBaseUrl, onPix, role, onLogout }) {
  const [itens, setItens] = React.useState([]);
  const [carregando, setCarregando] = React.useState(true);
  const [form, setForm] = React.useState({});
  const [editId, setEditId] = React.useState(null);
  ...
  // não muda o contrato da API (que os outros 9 projetos já usam tal como
  // está), só corta o que já foi carregado em páginas de 10.
  const itensFiltrados = itens.filter(item => !busca || ["loteId", "descricao", "categoria", "severidade", "acao", "responsavel", "prazo", "status", "id"].some(k => String(item[k] ?? "").toLowerCase().includes(busca.toLowerCase())));
  const totalPaginas = Math.max(1, Math.ceil(itensFiltrados.length / 10));
  const paginaAtual = Math.min(pagina, totalPaginas);
  const itensPagina = itensFiltrados.slice((paginaAtual - 1) * 10, paginaAtual * 10);

  const carregar = () => fetch(apiBaseUrl + "/api/naoconformidades", { headers: authHeaders(token) })
    .then(r => { if (r.status === 401) { onLogout && onLogout(); return []; } return r.json(); })
    .then(data => { if (Array.isArray(data)) setItens(data); })
    .catch(() => setItens([]))
    .finally(() => setCarregando(false));
  ...
  const salvar = (e) => {
    ...
    setErroSalvar("");
    setSalvando(true);
    const url = editId ? apiBaseUrl + "/api/naoconformidades/" + editId : apiBaseUrl + "/api/naoconformidades";
    fetch(url, {
      method: editId ? "PUT" : "POST",
      headers: { "Content-Type": "application/json", ...authHeaders(token) },
      body: JSON.stringify(montarCorpo()),
  ...
  const editar = (item) => {
    const f = {};
    ["loteId", "descricao", "categoria", "severidade", "acao", "responsavel", "prazo", "status"].forEach(k => { f[k] = item[k] ?? ""; });
    ["lote"].forEach(k => { f[k] = item[k + "Id"] ?? ""; });
    setErroSalvar(""); setForm(f); setEditId(item.id); setModalAberto(true);
  };

  const apagar = (id) => fetch(apiBaseUrl + "/api/naoconformidades/" + id, { method: "DELETE", headers: authHeaders(token) })
    .then(async r => {
      if (r.status === 401) { onLogout && onLogout(); return; }
      if (!r.ok) {
        const corpo = await r.json().catch(() => ({}));
  ...
  return (
    <div>
        ...
        <div className="kanban-board">
          {[...new Set(itensFiltrados.map(i => String(i.status ?? "Sem status")))].map(coluna => (
            <div className="kanban-coluna" key={coluna}>
              ...
              {itensFiltrados.filter(i => String(i.status ?? "Sem status") === coluna).map(item => (
                <div className="kanban-card" key={item.id}>
                  <div className="kanban-card-titulo">{String(item["loteId"] ?? "NaoConformidade")}</div>
                  <select className="kanban-select" value={item.status ?? ""} onChange={e => {
                    fetch(apiBaseUrl + "/api/naoconformidades/" + item.id, {
                      method: "PUT", headers: { "Content-Type": "application/json", ...authHeaders(token) },
                      body: JSON.stringify({ ...item, status: e.target.value }),
                    }).then(carregar);
                  }}>
      ...
      {!modoKanban && <><div className="items-grid">
        {itensPagina.map(item => (
          <div className="item-card" key={item.id}>
            ...
            <div className="item-title">{String(item["loteId"] ?? "NaoConformidade")}</div>
            <span className={"status-pill " + corStatus(item.status)}>{String(item.status ?? "")}</span>
            <div className="item-meta-grid">
            <div className="item-field"><b>descricao</b><span>{item["descricao"] ?? "—"}</span></div>
            <div className="item-field"><b>categoria</b><span>{item["categoria"] ?? "—"}</span></div>
            <div className="item-field"><b>severidade</b><span>{item["severidade"] ?? "—"}</span></div>
            <div className="item-field"><b>acao</b><span>{item["acao"] ?? "—"}</span></div>
            <div className="item-field"><b>responsavel</b><span>{item["responsavel"] ?? "—"}</span></div>
            <div className="item-field"><b>prazo</b><span>{item["prazo"] ? new Date(item["prazo"]).toLocaleString("pt-BR", {dateStyle:"short",timeStyle:"short"}) : "—"}</span></div>
            <div className="item-field"><b>lote</b><span>{item.loteId ? ((loteProducaoList.find(o => o.id === item.loteId) || {}).numero ?? ("#" + item.loteId)) : "—"}</span></div>
            </div>
            <div className="item-actions">
              <button className="btn btn-ghost btn-sm" onClick={() => editar(item)}>Editar</button>
      ...
      {modalAberto && (
        <div className="modal-overlay" onClick={() => setModalAberto(false)}>
          <div className="modal" onClick={e => e.stopPropagation()}>
            <h3>{editId ? "Editar" : "Novo"} NaoConformidade</h3>
            {erroSalvar && <div className="campo-erro modal-erro">{erroSalvar}</div>}
            <form onSubmit={salvar}>
              <label className="field-label">loteId</label>
      ...
      <label className="field-label">categoria</label>
      <input type="text" value={form["categoria"] ?? ""} onChange={e => setForm({...form, categoria: e.target.value})} />
      <label className="field-label">severidade</label>
      <input type="text" value={form["severidade"] ?? ""} onChange={e => setForm({...form, severidade: e.target.value})} />
      <label className="field-label">acao</label>
      <input type="text" value={form["acao"] ?? ""} onChange={e => setForm({...form, acao: e.target.value})} />
  ...
  return (
    <div className="app-shell">
      <aside className="sidebar">
        <div className="sidebar-logo">Controle Qualidade<span className="dot">.</span></div>
        <button className={"nav-btn" + (aba === "Produto" ? " active" : "")} onClick={() => setAba("Produto")}><span className="nav-ico">📦</span>Produto</button>
        <button className={"nav-btn" + (aba === "LoteProducao" ? " active" : "")} onClick={() => setAba("LoteProducao")}><span className="nav-ico">🗂️</span>LoteProducao</button>
        <button className={"nav-btn" + (aba === "InspecaoQualidade" ? " active" : "")} onClick={() => setAba("InspecaoQualidade")}><span className="nav-ico">📦</span>InspecaoQualidade</button>
        <button className={"nav-btn" + (aba === "NaoConformidade" ? " active" : "")} onClick={() => setAba("NaoConformidade")}><span className="nav-ico">🗂️</span>NaoConformidade</button>
      ...
      {aba === "LoteProducao" && <PainelLoteProducao token={token} apiBaseUrl={apiBaseUrl} onPix={abrirPix} role={role} onLogout={sair} />}
      {aba === "InspecaoQualidade" && <PainelInspecaoQualidade token={token} apiBaseUrl={apiBaseUrl} onPix={abrirPix} role={role} onLogout={sair} />}
      {aba === "NaoConformidade" && <PainelNaoConformidade token={token} apiBaseUrl={apiBaseUrl} onPix={abrirPix} role={role} onLogout={sair} />}
      </main>
